...
####
### Using quasipoisson distribution for comparison
### There is no "quasipoisson" option gor glmer, so we use the approach
### of adding individual level random effects.
...
for (i in 1:length(pi.names)){
  ...
  tmp.data$obs <- c(1:nrow(tmp.data))
  # get sample size
  ...
  if(pi.names[i] != "Wilson"){
    tmp.glmm <- glmer(sr ~ log10.tot.bio + I(log10.tot.bio^2) + (1|grid) + (1|obs),
